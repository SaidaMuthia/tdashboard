...
// search on filter
$("[name=branch], [name=cluster], [name=city], [name=subdistrict], [name=ward]").keyup(function() {
...
// filter item data
$("[name=region], [name=branch], [name=city], [name=subdistrict], [name=ward]").on('focus', function(obj) {
    ...
    if (multiSelectCount > 0 ){
        ...
        const fetchFilter = async function () {
            const { data: response } = await clientFilter.get("/teritory-filters", { ...configFilter, params: { field: getNameAttrValue, ...queryStringFilters } })
