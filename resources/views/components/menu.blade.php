...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme d-fixed">
  <div class="app-brand demo ps-0 pe-0">
    <a href="{{ url('/') }}" class="app-brand-link">
      <img width="100%" height="30px" src="{{ asset('assets/img/logo/logo-icon.png') }}" />
  ...
  <ul class="menu-inner py-1">
    ...
    @if ($menu['type']['level'] == 'main-menu')
    <li @class([ 'menu-item' , 'active'=> in_array($segment, array_merge([$menu['slug']], mainMenuToggle($menus, $menu['id']))),
      'open' => in_array($segment, array_merge([$menu['slug']], mainMenuToggle($menus, $menu['id']))),
      ])>
      <a href="{{ $menu['link'] }}" target="_self" @class([ 'menu-link' , 'menu-toggle'=> $menu['type']['hasSubmenu'] == true,
        ])>
        <i @class(['menu-icon', 'tf-icons' , $menu['icon']['content']])></i>
        <div class="text-truncate" data-i18n="{{ $menu['title'] }}">{{ $menu['title'] }}</div>
      </a>
      @if ($menu['type']['level'] == 'main-menu' && $menu['type']['hasSubmenu'] == true)
      <ul class="menu-sub">
        @foreach ($menus as $submenu)
        @if ($submenu['type']['level'] == 'sub-menu' && $submenu['type']['belongTo'] == $menu['id'])
        <li @class([ 'menu-item' , 'active'=> $segment == $submenu['slug']
          ]) >
          <a href="{{ $submenu['link'] }}" target="_self" class="menu-link">
    ...
    @endif
    {{-- check current iteration menu is main menu --}}
    @endforeach
    <!-- Layouts -->
    {{-- <li class="menu-item">
